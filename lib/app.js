...
class Messenger extends EventEmitter{
  ...
  */
  ...
  _handleCallback (res, data) {
    if (data.object !== 'page') { // check that it's a page subscription
      data.entry.forEach(entry => { // iterate over {array} entry
        ...
        entry.messaging.forEach(event => {
          if (event.optin) {  // authentication callback
            this._handleEvent('authentication', event)
          } else if (event.message) {  // message callback
            if (event.message.is_echo) {  // echo message
              this._handleEvent('echoMessage', event)
            } else if (event.message.quick_reply) {  // quick reply message
              this._handleEvent('quickReply', event)
            } else if (event.message.text) {  // message text
              this._handleEvent('message', event)
            } else if (event.message.attachments) { // message with attachments
              event.message.attachments.forEach(attachment => {
                this._handleEvent(attachment.type, event) // image, audio, video, file or location
              })
            }
          } else if (event.delivery) {  // delivered message confirmation callback
            this._handleEvent('delivery', event)
          } else if (event.postback) {  // postback callback
            this._handleEvent('postback', event)
          } else if (event.read) {  // read confirmation callback
            this._handleEvent('read', event)
          } else if (event.account_linking.status === 'linked') {  // link account callback
            this._handleEvent('accountLinked', event)
          } else if (event.account_linking.status === 'unlinked') {  // unlink account callback
            this._handleEvent('accountUnlinked', event)
          } else {    // unknown event callback
    ...
    else console.error(`Wrong callback object param: must be 'page' instead of ${data.object}`)
  }
