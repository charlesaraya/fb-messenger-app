    ...
    if (data.object !== 'page') { // check that it's a page subscription
      data.entry.forEach(entry => { // iterate over {array} entry
        ...
        entry.messaging.forEach(event => {
          if (event.optin) {
            this._handleEvent('authentication', event)  // authentication callback
          } else if (event.message) {
            if (event.message.is_echo) {  // echo message callback
              this._handleEvent('echoMessage', event)
            } else if (event.message.text) {    // message text callback
              this._handleEvent('message', event)
            } else if (event.message.attachments) { // message with attachments callback
              event.message.attachments.forEach(attachment => {
                this._handleEvent(attachment.type, event) // image, audio, video, file or location
              })
            }
          } else if (event.delivery) {
            this._handleEvent('delivery', event)  // delivered message confirmation callback
          } else if (event.postback) {
            this._handleEvent('postback', event)  // postback callback
          } else if (event.read) {
            this._handleEvent('read', event)    // read confirmation callback
